...
维护策略是守护兼容、不回退、不重复造轮子；研发资源优先投入 TermuxPro 增值服务，包括远程工作区、
...
每个新迭代切片先做增值服务准入检查：若不是直接提升远程开发、AI CLI、tmux/Git 可视化、自定义快捷
指令、移动端交互或工具箱体验，则不得进入产品研发队列。Termux 原本已有的基础能力只作为“不受影响”
回归面维护，不能被包装成 TermuxPro 功能研发目标。
